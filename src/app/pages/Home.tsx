      ...
      <div className="pt-10">
        ...
        <AnimatePresence>
          {!isPunoAlertDismissed && adjustedBranches.some((b) => b.is_puno) && (
            ...
            >
              <div className="bg-error dark:bg-red-950 text-on-error p-3 rounded-lg flex items-start gap-3 shadow-[0_8px_32px_rgba(25,28,30,0.06)] relative border border-outline-variant/10 dark:border-red-800">
                <span className="material-symbols-outlined flex-shrink-0 mt-0.5 text-on-error" style={{ fontVariationSettings: "'FILL' 1" } as any}>
                  warning
                </span>
                <div>
                  <h4 className="font-black text-[12px] uppercase tracking-widest mb-1 text-white">May PUNO na Branch</h4>
                  <p className="text-xs font-semibold opacity-90 leading-relaxed max-w-[90%]">
                    May {adjustedBranches.filter((b) => b.is_puno).length} branch malapit sa'yo na ubos na ang queue numbers ngayong araw. {/* IMPROVEMENT 1: PUNO state now comes from shared data. */}
                  </p>
                </div>
            ...
            {(decisionBranches.length ? decisionBranches : nearest2).map((b) => {
              ...
              return (
                <button
                  ...
                  className="flex-shrink-0 w-[155px] h-[110px] rounded-[12px] flex flex-col justify-start items-start border border-outline-variant/10 dark:border-slate-700/30 p-3 bg-surface-container-lowest dark:bg-slate-800 text-left transition-transform active:scale-[0.99] disabled:opacity-55 disabled:cursor-not-allowed"
                >
            ...
            <>
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleQuickStatClick('plastic')} className="bg-surface-container-lowest dark:bg-slate-800 p-6 rounded-lg shadow-[0_8px_32px_rgba(25,28,30,0.04)] border border-white/50 dark:border-slate-700/30 flex flex-col justify-between aspect-square text-left">
                ...
                <div>
                  ...
                  <div className="text-xs leading-tight text-on-surface-variant dark:text-slate-400">Branches with Plastic Cards</div>
                </div>
              </motion.button>
              ...
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleQuickStatClick('wait')} className="bg-surface-container-lowest dark:bg-slate-800 p-6 rounded-lg shadow-[0_8px_32px_rgba(25,28,30,0.04)] border border-white/50 dark:border-slate-700/30 flex flex-col justify-between aspect-square text-left">
                ...
                <div>
                  ...
                  <div className="text-xs leading-tight text-on-surface-variant dark:text-slate-400">Average Wait Time</div>
                </div>
              </motion.button>
              ...
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleQuickStatClick('flagged')} className="bg-surface-container-lowest dark:bg-slate-800 p-6 rounded-lg shadow-[0_8px_32px_rgba(25,28,30,0.04)] border border-white/50 dark:border-slate-700/30 flex flex-col justify-between aspect-square text-left">
                ...
                <div>
                  ...
                  <div className="text-xs leading-tight text-on-surface-variant dark:text-slate-400">Flagged Branches</div>
                </div>
              </motion.button>
              ...
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleQuickStatClick('reports')} className="bg-surface-container-lowest dark:bg-slate-800 p-6 rounded-lg shadow-[0_8px_32px_rgba(25,28,30,0.04)] border border-white/50 dark:border-slate-700/30 flex flex-col justify-between aspect-square text-left">
                ...
                <div>
                  ...
                  <div className="text-xs leading-tight text-on-surface-variant dark:text-slate-400">Total Reports Today</div>
                </div>
              </motion.button>
            </>
          )}
        ...
        <section className="mb-8 malapit-section">
          <div className="flex items-center justify-between px-1 mb-4">
            <h3 className="text-lg font-bold text-on-surface dark:text-slate-100">Mga Kalapit na Branch</h3>
          </div>
          <div className="space-y-4">
            ...
            {!isLoading && nearest2.map((b) => (
              <div key={b.id} onClick={() => openDetails(b)} className="p-4 bg-surface-container-low dark:bg-slate-800 rounded-2xl mb-4 border border-outline-variant/10 dark:border-slate-700/30 cursor-pointer hover:bg-surface-container transition-colors active:scale-[0.98]">
          ...
          {showAnomaly && (
            ...
            >
              <div className="max-w-[60%] space-y-2 relative z-10 w-full shrink-0">
                ...
                <p className="text-white/60 text-xs">Maging bahagi ng solusyon. Ipadala ang iyong karanasan sa kahit anong branch ng LTO.</p>
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full rounded-lg p-6 shadow-2xl border bg-surface-container-lowest dark:bg-slate-900 border-outline-variant/10 dark:border-slate-700/30">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-xs font-bold uppercase tracking-wider text-on-surface-variant">Lahat ng Branch Ngayon</p>
                  </div>
